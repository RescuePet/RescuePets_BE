...
//    @ApiOperation(value = "게시글 목록 조회", notes = "page, size, sortBy로 페이징 후 조회")
    @GetMapping("/")
    @Operation(summary = "Post 전체 게시글 불러오기", description = "PostCatch 전체 게시글을 페이징하여 불러옵니다")
    public ResponseEntity<ResponseDto> getPostList(@RequestParam int page,
                                                   @RequestParam int size,
                                                   @RequestParam(required = false, defaultValue = "createdAt") String sortBy,
                                                   @Parameter(hidden = true) @AuthenticationPrincipal MemberDetails memberDetails) {
        Member member = memberDetails.getMember();
        return postService.getPostList(page-1, size, sortBy, member);
    }
    @GetMapping("/all")
    @Operation(summary = "PostMissing 전체 게시글 페이징없이 불러오기", description = "PostMissing 전체 게시글을 페이징없이 불러옵니다")
    public ResponseEntity<ResponseDto> getPostAll(@RequestParam(required = false, defaultValue = "createdAt") String sortBy,
                                                  @Parameter(hidden = true) @AuthenticationPrincipal MemberDetails memberDetails) {
        Member member = memberDetails.getMember();
        return postService.getPostAll(sortBy, member);
    }
    @GetMapping("/member")
    ...
    public ResponseEntity<ResponseDto> getPostListByMember(@RequestParam int page,
                                                           ...
                                                           @Parameter(hidden = true) @AuthenticationPrincipal MemberDetails memberDetails) {
        Member member = memberDetails.getMember();
        return postService.getPostListByMember(page-1, size, sortBy, member);
    }
